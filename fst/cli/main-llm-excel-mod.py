import pandas as pd
import openpyxl
from openpyxl.cell.cell import TYPE_STRING, TYPE_NUMERIC, TYPE_BOOL, TYPE_FORMULA, TYPE_NULL, TYPE_ERROR
...
deployment = os.getenv("DEPLOYMENT_NAME", "gpt-4o") #gemini-pro-1.5 #claude-3-5-sonnet-v2 #gpt-4o
...
def gen_ai_translate_text(value, company):
    if isinstance(value, str) and bool(re.fullmatch(r'[\d,.\-\s]+', value)) == False and value.strip():
        return open_ai_translate(value, company)
    return value
def open_ai_translate(value, company):
    response = client.chat.completions.create(
        model=deployment,
        user="gpt-ptt-excel-2",
        ...
        # tags=["bem", "xlsx", "model:{deployment}"],
        messages=[
...
def get_translated_text(cell, company, jargon_list_json):
    jargon_word = jargon_list_json.get(cell.value, None)
    if jargon_word != None:
        translated_text = jargon_word
    else: 
        translated_text = gen_ai_translate_text(cell.value, company)
    ...
    for sheet in wb.worksheets:
        ...
        for row in sheet.iter_rows():
            ...
            for cell in row:
                ...
                if cell.data_type == TYPE_FORMULA or cell.data_type == TYPE_NULL or cell.data_type == TYPE_ERROR or cell.data_type == TYPE_BOOL or cell.data_type == TYPE_NUMERIC:
                    continue
                ...
                for merged_range in merged_ranges:
                    if cell.coordinate in merged_range:
                        ...
                        if cell.coordinate == top_left_cell.coordinate and isinstance(cell.value, str):
                            cell.value = get_translated_text(cell, company, jargon_list_json)
                        break
                else:
                    cell.value = get_translated_text(cell, company, jargon_list_json)
...
if __name__ == "__main__":
    ...
    # Load Excel
    file_path = "./data/ptt/FINANCIAL_STATEMENTS.XLSX"
    output_path = "translated_ptt_sheet3-gpt-4o.xlsx" #claude-3-5-sonnet-v2 #gemini-pro-1.5 #gpt-4o
